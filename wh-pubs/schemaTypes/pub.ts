...
export default defineType({
  ...
  fields: [
          ...
          fields: [
            {
              ...
              title: 'Title',
              type: 'string',
              description: 'e.g., "SHACKS" or "LOCAL ALES"',
              validation: (Rule) => Rule.max(30),
            },
            {
              name: 'description',
          ...
          fields: [
            {
              ...
              title: 'Title',
              type: 'string',
              description: 'e.g., "PRIVATE DINING" or "SPECIAL EVENTS"',
              validation: (Rule) => Rule.max(30),
            },
            {
              name: 'description',
    ...
    defineField({
      name: 'pubWebsiteUrl',
      title: 'Pub Website URL',
      type: 'url',
      group: 'basic',
      description: '🌐 The main website URL for this pub (will be used for "Visit Pub Site" button). Once approved by the landlord, this should be updated from the temporary Netlify URL to the final domain.',
      placeholder: 'https://whpubs-thebull.netlify.app/ or https://www.thebullpub.com',
    }),
    defineField({
      name: 'temporaryUrl',
      title: 'Temporary/Development URL',
      type: 'url',
      group: 'basic',
      description: '🚧 Temporary Netlify URL used during development (e.g., https://whpubs-thebull.netlify.app/). This will be replaced by the final domain once approved.',
      placeholder: 'https://whpubs-thebull.netlify.app/',
    }),
    defineField({
      name: 'url',
      title: 'Legacy Website URL (Deprecated)',
      type: 'url',
      group: 'basic',
      description: '⚠️ DEPRECATED: Use "Pub Website URL" instead. This field is kept for backwards compatibility.',
      hidden: true,
    }),
    defineField({
      name: 'externalDomain',
      title: 'External Domain (Deprecated)',
      type: 'string',
      group: 'basic',
      description: '⚠️ DEPRECATED: Use "Pub Website URL" instead. This field is kept for backwards compatibility.',
      hidden: true,
      ...
      fields: [
        {
          ...
          type: 'string',
          title: 'Alternative text',
          description: 'Describe the image for screen readers and SEO (Optional for mobile services)',
          validation: (Rule) => Rule.custom((alt, context) => {
            // Make alt text optional for mobile services
            if (context.document?.locationName === 'Mobile Service' || 
                context.document?.locationName === 'Anywhere' ||
    ...
    defineField({
      ...
      fields: [
        { 
          ...
          type: 'string', 
          title: 'Alternative text',
          description: 'Describe this hero image (Optional for mobile services)',
          validation: (Rule) => Rule.custom((alt, context) => {
            // Make alt text optional for mobile services
            if (context.document?.locationName === 'Mobile Service' || 
                context.document?.locationName === 'Anywhere' ||
